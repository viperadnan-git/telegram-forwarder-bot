...
const id = (input: string) => {
    ...
    if (!r.ok || r.ref.kind !== "id")
        throw new Error(`expected id, got ${JSON.stringify(r)}`);
    return r.ref.id;
};
...
describe("raw ids", () => {
    test("negative channel id", () =>
        expect(id("-1001234567890")).toBe(-1001234567890));
    test("positive user id", () => expect(id("12345")).toBe(12345));
    test("surrounding whitespace", () =>
        expect(id("  -100123  ")).toBe(-100123));
...
describe("usernames", () => {
    ...
    test("underscores and digits", () =>
        expect(username("my_chan_99")).toBe("my_chan_99"));
    test("too short is rejected", () => expect(fails("@ab")).toBe(true));
    test("leading digit is rejected", () =>
        expect(fails("@1channel")).toBe(true));
    test("empty is rejected", () => expect(fails("   ")).toBe(true));
});

describe("public links", () => {
    test("chat link", () =>
        expect(username("https://t.me/durov")).toBe("durov"));
    ...
    test("with www", () =>
        expect(username("https://www.t.me/durov")).toBe("durov"));
...
describe("private links", () => {
    ...
    test("missing id is rejected", () =>
        expect(fails("https://t.me/c/")).toBe(true));
});
...
describe("unsupported", () => {
    ...
    test("non-telegram host", () =>
        expect(fails("https://example.com/durov")).toBe(true));
    test("bare t.me with no path", () =>
        expect(fails("https://t.me/")).toBe(true));
});
